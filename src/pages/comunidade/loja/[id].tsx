...
type ConversaLoja = {
  id: string;
  compradorId: string;
  comprador: { id: string; name: string; apartamento: string | null; bloco: string | null };
...
function ConversaMsgs({
  ...
  const fetchMsgs = useCallback(async () => {
    const res = await fetch(`/api/comunidade/loja/conversas/${conversaId}`);
...
function PainelComprador({ anuncioId, userId }: { anuncioId: string; userId: string }) {
  ...
  useEffect(() => {
    fetch(`/api/comunidade/loja/conversas?anuncioId=${anuncioId}`)
      .then((r) => r.json())
      .then((data) => {
  ...
  const handleIniciar = async () => {
    if (!texto.trim()) return;
    setEnviando(true);
    const res = await fetch("/api/comunidade/loja/conversas", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ anuncioId, conteudo: texto.trim() }),
    });
    setEnviando(false);
    if (res.ok) {
  ...
  if (conversaId) {
    return <ConversaMsgs conversaId={conversaId} userId={userId} />;
  }

  return (
    <div className="space-y-3">
      <p className="text-sm text-muted-foreground">Inicie a conversa com o vendedor.</p>
      <textarea
        ...
        value={texto}
        onChange={(e) => setTexto(e.target.value)}
        placeholder="Olá! Tenho interesse neste item…"
      />
      <Button
...
function PainelVendedor({ anuncioId, userId }: { anuncioId: string; userId: string }) {
  const [conversas, setConversas] = useState<ConversaLoja[]>([]);
  const [ativa, setAtiva] = useState<ConversaLoja | null>(null);

  const fetchConversas = useCallback(async () => {
    const res = await fetch(`/api/comunidade/loja/conversas?anuncioId=${anuncioId}`);
  ...
  if (ativa) {
    return (
      <div>
        ...
        <p className="font-medium text-sm mb-3">{ativa.comprador.name}</p>
        ...
        return (
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              <p className="text-sm font-medium">{c.comprador.name}</p>
              {ultima && (
                <p className="text-xs text-muted-foreground truncate">{ultima.conteudo}</p>
              )}
            </div>
...
export default function AnuncioDetailPage({ anuncio, userId }: Props) {
  ...
  const handleStatus = async (status: string) => {
    const res = await fetch(`/api/comunidade/loja/${anuncio.id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
  ...
  return (
    <AppShell titulo={anuncio.titulo}>
      <div className="max-w-2xl mx-auto px-4 sm:px-6 py-8">
        ...
        {anuncio.fotoUrl && (
          <img
            src={anuncio.fotoUrl}
            alt={anuncio.titulo}
            className="w-full max-h-96 object-cover rounded-xl mb-6 border border-border"
          />
        )}

        <div className="flex flex-wrap gap-2 mb-3">
          <span className={cn("px-2.5 py-1 rounded-full text-xs font-medium", tpCfg.cls)}>
            {tpCfg.label}
          </span>
          <span className={cn("px-2.5 py-1 rounded-full text-xs font-medium", stCfg.cls)}>
            {stCfg.label}
          </span>
        ...
        <h1 className="text-2xl font-bold mb-2">{anuncio.titulo}</h1>

        {anuncio.valor != null ? (
          <p className="text-2xl font-bold text-indigo-600 mb-4">{formatBRL(anuncio.valor)}</p>
        ) : anuncio.tipo === "DOACAO" ? (
          <p className="text-xl font-semibold text-rose-600 mb-4">Gratuito</p>
        ) : null}

        <p className="text-sm text-foreground whitespace-pre-wrap mb-6">{anuncio.descricao}</p>
        ...
        <div className="flex items-center gap-3 p-4 bg-muted/50 rounded-xl mb-6 border border-border">
          ...
          <div>
            <p className="font-medium text-sm">{anuncio.morador.name}</p>
        ...
        {isOwner && (
          <div className="flex flex-wrap gap-2 mb-8 pb-6 border-b border-border">
            {anuncio.status === "DISPONIVEL" && (
              <Button size="sm" variant="outline" onClick={() => handleStatus("RESERVADO")}>
                Marcar como Reservado
              </Button>
            )}
            {anuncio.status === "RESERVADO" && (
              <>
                <Button size="sm" variant="outline" onClick={() => handleStatus("VENDIDO")}>
                  Concluir Venda
                </Button>
                <Button size="sm" variant="outline" onClick={() => handleStatus("DISPONIVEL")}>
                  Reabrir
                </Button>
              </>
            )}
        ...
        <div className="border border-border rounded-xl p-5">
          <h2 className="font-semibold mb-4">
            {isOwner ? "Interessados" : "Chat com o Vendedor"}
          </h2>
          {isOwner ? (
            <PainelVendedor anuncioId={anuncio.id} userId={userId} />
          ) : (
            <PainelComprador anuncioId={anuncio.id} userId={userId} />
          )}
        </div>
      </div>
...
export const getServerSideProps: GetServerSideProps = async (context) => {
  ...
  if (!anuncio) return { notFound: true };

  return {
    props: {
      anuncio: {
        ...anuncio,
        criadaEm: anuncio.criadaEm.toISOString(),
        atualizadaEm: anuncio.atualizadaEm.toISOString(),
      },
